			   ...
			   const char *name, type t)
{
  ...
  dd->defined = FALSE;
  dd->isallocated = FALSE;
  dd->addresstaken = FALSE;
  dd->ftype = 0;
  dd->isinline = FALSE;
  ...
  if (!is_void_parms(parameters))
      ...
      if (is_ellipsis_decl(parameter))
	{
	  ...
	  *varargs_quals = parse_qualifiers(parameter->loc,
				   CAST(ellipsis_decl, parameter)->qualifiers,
					    varargs_user_quals);
	}
				   ...
				   user_qual_list *user_quals_out)
{
  ...
  user_qual_list user_quals = NULL;

  /* Actually qlist is a list of qualifier */
  scan_type_element (qual, qlist)
    {
      switch (qual->kind)
	{
	case kind_qualifier:
	  {
	    qualifier q = CAST(qualifier, qual);
	    check_duplicate_qualifiers1(q->loc, q->id, tqs);
	    tqs |= q->id;
	  }
	  break;
	case kind_user_qual:
	  user_quals = new_user_qual_list(CAST(user_qual, qual),
					  user_quals);
	  break;
	default: assert(0);
	}
  ...
  *oclass = 0;
  ...
  scan_type_element (spec, modifiers)
	...
	case kind_struct_ref: case kind_union_ref: case kind_enum_ref:
	  ...
	  break;
	case kind_user_qual:
	  user_quals = new_user_qual_list(CAST(user_qual, spec), user_quals);
	  break;
	case kind_type_variable:
	  newtype = make_type_var(CAST(type_variable, spec)->cstring);
	  break;
	...
	case kind_function_declarator:
	  {
		...
		t = make_function_type(t, argtypes, varargs,
	      ...
	      t = make_function_type(t, NULL, FALSE, no_qualifiers, NULL,TRUE);
			   ...
			   bool different_binding_level, bool newinitialised)
{
  ...
  if (olddecl->alias == NULL)
    olddecl->alias = newdecl->alias;
  assert(newdecl->alias == NULL || !strcmp(olddecl->alias, newdecl->alias));
...
static void transparent_union_argument(data_declaration ddecl)
{
  ddecl->type = make_qualified_type
    (ddecl->type, type_qualifiers(ddecl->type) | transparent_qualifier, NULL);
}
		      ...
		      field_declaration fdecl, tag_declaration tdecl)
{
  ...
  else if (!strcmp(attr->word1->cstring.data, "noreturn") ||
	   !strcmp(attr->word1->cstring.data, "__noreturn__"))
    {
      if (ddecl)
	ddecl->noreturn = TRUE;
      else
  ...
  else if (!strcmp(attr->word1->cstring.data, "alias") ||
	   !strcmp(attr->word1->cstring.data, "__alias__"))
    {
      if (attr->args && is_string(attr->args))
	{
	  string str = (string)attr->args;
	  ddecl->alias = rstralloc(parse_region, str->ddecl->chars_length);
	  wcstombs(ddecl->alias, str->ddecl->chars, str->ddecl->chars_length);
	}
    }	   
  else if (!strcmp(attr->word1->cstring.data, "section") ||
	   !strcmp(attr->word1->cstring.data, "__section__"))
    {
      if (attr->args && !attr->args->next &&
	  attr->args->kind == kind_string &&
	  !strcmp(string_to_charp(parse_region,
				  CAST(string, attr->args)),
		  ".text.init"))
	{
	  if (ddecl)
	    ddecl->__init = TRUE;
...
static void declare_builtin_identifiers(void)
{
  ...
  type default_function_type = make_function_type(int_type, emptylist, FALSE, no_qualifiers, NULL, TRUE);
...
data_declaration implicitly_declare(identifier fnid)
{
  ...
  declaration pseudo_ast =
    CAST(declaration, new_implicit_decl(parse_region, fnid->loc, fnid));
...
{
  variable_decl vd = 
    new_variable_decl(parse_region, d->loc, d, extra_attributes, NULL,
		      astmt, NULL);
  struct data_declaration tempdecl;
			   ...
			   attribute attribs)
{
  ...
  scan_declaration (fdecl, fields)
      ...
      scan_field_decl (field, CAST(field_decl, flist->decls))
	{
	  ...
	  else if (type_incomplete(field_type) && field->next) 
	    {
...
void init_semantics(void)
{
  ...
  implicit_function_type = make_function_type(int_type, NULL, FALSE, no_qualifiers, NULL, TRUE);
