...
%expect 50
/* Added 4 conflicts from base system for USER_QUALIFIER */
...
/* A user-defined type qualifier */
%token USER_QUALIFIER
...
%token <u.itoken> ASSERT_TYPE CHANGE_TYPE DEEP_RESTRICT
...
%type <u.decl> parmlist parmlist_1 parmlist_2 parms parm ellipsis_decl
...
%type <idtoken> identifier IDENTIFIER TYPENAME USER_QUALIFIER TYPE_VARIABLE MAGIC_STRING
...
program: /* empty */
	...
	| extdefs
		{
		  the_program = CAST(declaration, ast_reverse(CAST(node, $1)));
		}
	;
...
datadef:
	  setspecs notype_initdecls ';'
		{ if (pedantic)
		  ...
		  $$ = CAST(declaration, new_data_decl(pr, $2->loc, NULL, NULL, $2));
		  pop_declspec_stack(); }
        | declmods setspecs notype_initdecls ';'
		{ $$ = CAST(declaration, new_data_decl(pr, $1->loc, current_declspecs, prefix_attributes, $3));
		  pop_declspec_stack(); }
	...
	| typed_declspecs setspecs ';'
	  { shadow_tag($1); 
	    $$ = CAST(declaration, new_data_decl(pr, $1->loc, current_declspecs, prefix_attributes, NULL));
	    pop_declspec_stack(); }
...
cast_expr:
	...
	| '(' typename ')' '{' 
	  ...
	  initlist_maybe_comma '}'  %prec UNARY
		{ 
		  $$ = CAST(expression, new_cast_list(pr, $1.location, $2, CAST(expression, new_init_list(pr, $6->loc, $6))));
		  $$->type = $2->type;
		  $$->lvalue = TRUE;
...
primary:
	...
	| primary '.' identifier
		{ $$ = make_field_ref($2.location, $1, $3.id, $3.location); }
...
string:
	string_list { $$ = make_string($1->loc, $1); }
...
ellipsis_decl:
	ELLIPSIS
          { $$ = CAST(declaration, new_ellipsis_decl(pr, $1.location, NULL)); }
      | type_qual ELLIPSIS
          { $$ = CAST(declaration, new_ellipsis_decl(pr, $2.location, $1)); }
      | ELLIPSIS type_qual
          { $$ = CAST(declaration, new_ellipsis_decl(pr, $1.location, $2)); }
      | type_qual ELLIPSIS type_qual
          { $$ = CAST(declaration,
		      new_ellipsis_decl(pr, $2.location,
					type_element_chain($1, $3))); }
...
typespec: type_spec
	...
	| TYPENAME
        ...
        | TYPE_VARIABLE
                { $$ = CAST(type_element, new_type_variable(pr, $1.location, $1.id)); }
	| TYPEOF '(' expr ')'
...
attrib:
    /* empty */
		{ $$ = NULL; }
	| any_word
		{ $$ = new_attribute(pr, $1->loc, $1, NULL, NULL); }
	| any_word '(' IDENTIFIER ')'
		{ $$ = new_attribute
		    (pr, $1->loc, $1, new_word(pr, $3.location, $3.id), NULL); }
...
any_word:
	  idword
	| scspec { $$ = new_word(pr, $1->loc, str2cstring(pr, rid_name(CAST(rid, $1)))); }
	| type_spec { $$ = new_word(pr, $1->loc, str2cstring(pr, rid_name(CAST(rid, $1)))); }
	| type_qual { $$ = new_word(pr, $1->loc, str2cstring(pr, qualifier_name(CAST(qualifier, $1)->id))); }
	;

...
initlist_maybe_comma:
	  /* empty */
		{ if (pedantic)
		  ...
		  $$ = NULL; }
	| initlist1 maybecomma { $$ = CAST(expression, ast_reverse(CAST(node, $1))); }
	;
...
initelt:
	...
	| idword ':' initelt
	    	{ $$ = CAST(expression, new_init_field(pr, $1->loc, $1, $3)); }
	| '.' idword '=' initelt
...
simple_if:
	  if_prefix labeled_stmt
		{ $$.stmt = CAST(statement, new_if_stmt(pr, $1.location, $1.expr, $2, NULL));
		  $$.i = $1.i; }
...
do_stmt_start:
	  DO
		{ stmt_count++;
		 ...
		 push_loop(CAST(breakable_stmt, $<u.cstmt>$)); }
	  labeled_stmt WHILE
		{ $$ = $<u.cstmt>2; 
		  $$->stmt = $3; }
	;
...
labeled_stmt:
	  stmt
		{ $$ = $1; }
	| label labeled_stmt
		{ $$ = CAST(statement, new_labeled_stmt(pr, $1->loc, $1, $2)); }
	;
...
stmt_or_label:
	  stmt
		{ $$.i = 0; $$.stmt = $1; }
	| label
		{ $$.i = 1; $$.stmt = CAST(statement, new_labeled_stmt(pr, $1->loc, $1, NULL)); }
	;
...
stmt:
	...
	| expr ';'
		{ stmt_count++;
		  $$ = CAST(statement, new_expression_stmt(pr, $1->loc, $1)); }
	| simple_if ELSE
		{ $1.i = stmt_count; }
	  labeled_stmt
	...
	| simple_if %prec IF
		{ /* This warning is here instead of in simple_if, because we
		  ...
		  if (extra_warnings && stmt_count++ == $1.i)
		    warning_with_location ($1.stmt->loc,
	...
	| FOR '(' xexpr ';' { stmt_count++; }
		...
		{ $<u.for_stmt>$ = new_for_stmt(pr, $1.location, $3, $6, $9, NULL);
		  push_loop(CAST(breakable_stmt, $<u.for_stmt>$)); }
		labeled_stmt
	...
	| ';' { $$ = CAST(statement, new_empty_stmt(pr, $1.location)); }
        | ASSERT_TYPE '(' unary_expr ',' typename ')' ';'
        { $$ = CAST(statement, new_assert_type_stmt(pr, $1.location, $3, $5)); }
        | CHANGE_TYPE '(' unary_expr ',' typename ')' ';'
        { $$ = CAST(statement, new_change_type_stmt(pr, $1.location, $3, $5)); }
        | DEEP_RESTRICT '(' expr ')' labeled_stmt
        { $$ = CAST(statement, new_deep_restrict_stmt(pr, $1.location, $3, $5)); }
	;
...
asm_operand:
	  STRING '(' expr ')'
		{ $$ = new_asm_operand(pr, $1->loc,
				       make_string($1->loc, CAST(expression, $1)),
				       $3);  }
	;
...
type_qual:
	TYPE_QUAL { $$ = CAST(type_element, new_qualifier(pr, $1.location, $1.i)); }
       | USER_QUALIFIER { $$ = CAST(type_element,
			       new_user_qual(pr, $1.location, $1.id)); }
	;
