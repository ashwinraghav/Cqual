  ...
  */
  ...
  union {
    ...
    struct {
      type returns;
      typelist argtypes;
      type_quals varargs_quals;          /* allow quals on varargs for cqual */
			 ...
			 user_qual_list user_quals)
{
  ...
  if (t->kind == tk_array)
  ...
  else
    {
      ...
      /* nt is a fresh copy, so we can just chain the lists together */
...
/* Return type 'function with argument types argtypes returning t'.
   ...
   argtypes is NULL.  Allow qualifiers on ... for cqual. */
			    ...
			    type_element rest)
{
  if (uql)
    {
      type_element ast = CAST(type_element,
			      new_user_qual(r, loc, uql->qual->cstring));
      ast->next = CAST(node, uql2ast(r, loc, uql->next, rest));
      return ast;
    }
  else
    return rest;
	  ...
	  if (t->u.fn.varargs)
	    {
	      ...
	      vquals = qualifiers2ast(r, loc, t->u.fn.varargs_quals,
				      t->u.fn.varargs_user_quals, NULL);
	      *lastparm = CAST(declaration,
			       new_ellipsis_decl(r, loc, vquals));
	      lastparm = (declaration *)&(*lastparm)->next;
	    }
	  *lastparm = NULL;
	}
