/* This file was modified in 2000-2002 by David Gay and Jeff Foster for cqual.
   The changes are 
...
/* alloca tests from autoconf man page */
...
#define GETC() (yy_cur < yy_lim ? (filepos++, *yy_cur++) : yy_get_token ())
#define UNGETC(c) ((c), filepos--, yy_cur--)
#else
#define GETC() (filepos++, getc (finput))
#define UNGETC(c) (filepos--, ungetc (c, finput))
#endif
...
init_lex ()
{
  dummy_location = ralloc(parse_region, struct Location);
  dummy_location->filename = "<dummy>";

  /* Start it at 0, because check_newline is called at the very beginning
     and will increment it to 1.  */
  lineno = 0;
  filepos = -1;  /* Start at -1, since we haven't read any input */
...
check_newline ()
{
  int c;
  int token;
  int old_filepos = filepos; /* Before we hit any #line directives */
      ...
      if (token_isint(token))
	{
	  ...
	  if (cst == 1)
		...
		= (struct file_stack *) xmalloc (sizeof (struct file_stack));
	      ...
	      input_file_stack->filepos = old_filepos;
  ...
  switch (c)
	...
	*token_ptr = '\0';
	if ((ptr = is_reserved_word (token_buffer, token_ptr - token_buffer)))
      ...
      /* If we did not find a keyword, look for an identifier, or
	 a typename, or a user-defined qualifier */
      if (value == IDENTIFIER)
	{
	  ...
	  if (!yylval.idtoken.decl && token_buffer[0] == '$' &&
	      token_buffer[1] == '$')
	    value = TYPE_VARIABLE;
	  else if (!yylval.idtoken.decl && token_buffer[0] == '$')
	    /* If it hasn't been defined, then it's a type qualifier */
	    value = USER_QUALIFIER;
