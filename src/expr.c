			    ...
			    expression true, expression false)
{
  ...
  if (ctype == error_type || ttype == error_type || ftype == error_type)
    ...
    rtype = make_qualified_type(ttype, no_qualifiers, NULL);
...
expression make_assign(location loc, int binop, expression e1, expression e2)
{
  ...
  if (t1 != error_type && e2->type != error_type)
	...
	result->type = make_qualified_type(e1->type, no_qualifiers, NULL);
    }
			  ...
			  location cstring_loc)
{
  ...
  expression result = CAST(expression, new_field_ref(parse_region, loc, object, field, cstring_loc));
